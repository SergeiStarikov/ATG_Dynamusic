...
public class SongsManager extends GenericService {
   ...
   */
   ...
   /** 
    ** This method implements a jazzSongs property, a read-only property
    ** which uses a named query called 'jazzSongs' defined in songs.xml.
    **/
   public RepositoryItem[] getJazzSongs() {

     try {  	
       RepositoryView songView = getRepository().getView("song");
       if(songView instanceof NamedQueryView) {
     	
  	 NamedQueryView nameView = (NamedQueryView)songView;
  	 Query namedQuery = nameView.getNamedQuery("jazzSongs");
  	 ...
  	 return results;
     	  ...
     	  if (isLoggingError())
   	     logError("could not retrieve jazz songs", re);
     }
     return null;
   }
